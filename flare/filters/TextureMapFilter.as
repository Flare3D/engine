package flare.materials.filters
{
	 ...
	 * @author Ariel Nehmad
	 */
	public class TextureMapFilter extends FLSLFilter
	{
		[Embed( source="../shaders/textureMap.flsl.compiled",mimeType="application/octet-stream" )]
		private static var flsl:Class;
		private static var data:ByteArray = new flsl;
		
		public function TextureMapFilter( texture:Texture3D = null, channel:int = 0, blendMode:String = BlendMode.MULTIPLY, alpha:Number = 1 )
		{
			super( data, blendMode, "main" );
			
			params.alpha.value = Vector.<Number>([ alpha ]);
		...
		override public function init( material:Material3D, index:int, pass:int ):void
		{
			if ( params.mask.value && params.mask.value[ 0 ] > 0 )
				material.flags |= Material3D.FLAG_MASK;
		}
